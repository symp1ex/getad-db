...
// Функция для генерации и загрузки JSON-файла
function downloadTXT(data, filename) {
	var text = data.toString(); // Преобразуем данные в строку
	var blob = new Blob([text], { type: "text/plain" }); // Создаем Blob с типом text/plain
	var url = URL.createObjectURL(blob); // Создаем URL для Blob
	var a = document.createElement("a"); // Создаем элемент <a>
	a.href = url; // Устанавливаем href на созданный URL
	a.download = filename; // Устанавливаем имя файла для скачивания
	document.body.appendChild(a); // Добавляем элемент <a> в документ
	a.click(); // Имитируем клик по элементу <a>
	document.body.removeChild(a); // Удаляем элемент <a> из документа
	URL.revokeObjectURL(url); // Освобождаем память, удаляя URL
}
...
function applyFilter() {
    ...
    if (currentColumnIndex >= 0) {
        ...
        if (headers[currentColumnIndex + 1]) { // +1 из-за столбца с чекбоксами
            headers[currentColumnIndex + 1].classList.add('filtered-column');
        }
    }
...
function clearFilter() {
    if (currentColumnIndex >= 0) {
        ...
        const headers = document.querySelectorAll('th');
        if (headers[currentColumnIndex + 1]) {
            headers[currentColumnIndex + 1].classList.remove('filtered-column');
        }
    }
...
function applyAllFilters() {
    ...
    rows.forEach(row => {
        ...
        Object.keys(activeFilters).forEach(columnIndex => {
            ...
            const cellText = row.cells[parseInt(columnIndex) + 1].textContent.toLowerCase(); // +1 из-за чекбоксов
